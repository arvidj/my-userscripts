...
// ==UserScript==
// @name             Gmail Google Calendar sidebar
// @namespace        http://www.r2unit.com/greasemonkey
// @description      Adds a sidebar to GMail that displays your upcoming events/appointments from Google Calendar.
...
GM_addStyle(
            // Block in sidebar
            ".gcalDIV {-moz-border-radius: 5px; background: #c3d9ff; margin: 20px 7px 0 0; padding: 3px;}\n"+
...
gEditA.href = "http://gmail.com";
gEditA.className = "lk cs gcalEditLink";
gEditA.appendChild(newText("Reload"));
...
function getData(){
   var d = new Date();
   if (parseInt(GM_getValue("last_updated", 0)) + 3600000 > d.getTime()){
      var json = GM_getValue("events", "");
      try {
         data = eval(json);
         for (var i=0; i < data.length; i++){
            e = data[i];
            addEvent(new CalendarEvent(e.id,e.title,e.startTS,e.endTS,e.URL,e.location,e.repeatTS,e.repeatType,e.repeatInt,e.repeatByDay, e.colour));
         }
...
function makeJSON(events){
   ...
   for (var i = 0; i < events.length; i++){
      ...
      result += "colour : '"+ event.colour + "',"
      result += "startTS : "+ event.startTS + ","
      result += "endTS : "+ event.endTS + ","
      result += "duration : "+ event.duration + ","
      result += "repeatTS : '"+ event.repeatTS + "',"
      result += "repeatType : '"+ event.repeatType + "',"
      result += "repeatInt : '"+ event.repeatInt + "',"
      result += "repeatByDay : '"+ event.repeatByDay + "'"
      result += "}"
      results.push(result);
   }
   return "[" + results.join(",") + "]";
...
function getFeed(gcalURL, setTitle){
   ...
   GM_xmlhttpRequest({
      ...
      onload: function(responseDetails) { parseCalendarXML(responseDetails.responseText, setTitle); },
      onerror: function(responseDetails) { gcalStatusDIV.innerHTML = "Error Loading Agenda"; }
   });
}

function parseCalendarXML(feedXML, setTitle){
      ...
      if(!evCancelled && !aParent[evId]){ 
         ...
         for (var aWheni = 0; aWheni < aEvent.length; aWheni++){ 
            if (new Date(aEvent[aWheni]['evEndTS']).getTime() != aEvent[aWheni]['evEndTS']) aEvent[aWheni]['evEndTS'] = aEvent[aWheni]['evstartTS'];
               addEvent(new CalendarEvent(evId,evTitle,aEvent[aWheni]['evStartTS'],aEvent[aWheni]['evEndTS'],evURL,evLocation,evRepeatTS,evRepeatType,evRepeatInt,evRepeatByDay, calColour));
         }
      } 
   ...
   EventList = EventList.sort(function(a,b){return cmp(a.startTS,b.startTS);});
   renderCalendar(EventList); 
   GM_setValue("events", makeJSON(EventList));
   var date = new Date();
   GM_setValue("last_updated", date.getTime().toString());
...
function renderCalendar(eventA){
   ...
   for (var eI = 0; eI < eCount; eI++){
      ...
      if (std['d'] != etd['d']){
         ...
         summary = event.title;
         evHTML += "<TR><TD colspan=" + colspan + "><A class='gcalItem' style='color:" + event.color + "; font-weight: bold;' href='" + event.URL   + "' target='_blank'>" + summary + "</A></TD></TR>";
         if (Dlocation && event.location != '') evHTML += "<TR><TD " +   style + "style='color:" + event.colour + ";'" +" colspan=" + colspan + ">&nbsp;" + event.location + "</TD></TR>";

         evHTML += "<TR>" 
                   + "<TD " +   style + " style='color:" + event.colour + ";'>&nbsp;" + std['day'] + "</TD>" 
                   + "<TD " +   style + " style='color:" + event.colour + ";'>" + std['month'] + "</TD>" 
                   + "<TD " +   style + " style='color:" + event.colour + ";'>" + std['date'] + "</TD>";

         if (!Dsingle){
            evHTML += "<TD " +   style + " style='color:" + event.colour + ";'>" + std['year'] + "</TD>";
            evHTML += "<TD " +   style + " width='100%' style='color:" + event.colour + ";'>&nbsp;" + std['t'] + "</TD></TR>";
         } else {
            evHTML += "<TD " +   style + " width='100%' style='color:" + event.colour + ";'>" + std['remainDate'] + "</TD></TR>";
            if (std['t'] != '') evHTML += "<TR><TD " +   style + " colspan=" + colspan + " style='color:" + event.colour + ";'>&nbsp;" + std['t'] + "</TD></TR>";
         }

         if (!Dsingle){
            evHTML += "<TR>" 
                      + "<TD " +   style + " style='color:" + event.colour + ";'>&nbsp;" + etd['day'] + "</TD>" 
                      + "<TD " +   style + " style='color:" + event.colour + ";'>" + etd['month'] + "</TD>" 
                      + "<TD " +   style + " style='color:" + event.colour + ";'>" + etd['date'] + "</TD>" 
                      + "<TD " +   style + " style='color:" + event.colour + ";'>" + etd['year'] + "</TD>"
                      + "<TD " +   style + " width='100%' style='color:" + event.colour + ";'>&nbsp;" + etd['t'] + "</TD></TR>";
         }
         ...
         colspan = (!Dsingle) ? 5 : 6;
         summary = event.title;
         evHTML += "<TR><TD colspan=" + colspan + "><A class='gcalItem' style='color:" + event.colour +"; font-weight: bold;';href='" + event.URL   + "' target='_blank'>" + summary + "</A></TD></TR>";
         if (Dlocation && event.location != '') evHTML += "<TR><TD " +   style + "style='color:" + event.colour + ";'" +" colspan=" + colspan + ">&nbsp;" + event.location + "</TD></TR>";
         ...
         if (!Dsingle){
            evHTML += "<TD " +   style + " style='color:" + event.colour + ";'>" + etd['year'] + "</TD>";
            evHTML += "<TD " +   style + "   width='100%' style='color:" + event.colour + ";'>&nbsp;" + std['t'] + "</TD></TR>";
         } else {
            evHTML += "<TD " +   style + " style='color:" + event.colour + ";'>" + std['year'] + "</TD></TR>";
            if (std['t'] != '') evHTML += "<TR><TD " +   style + " colspan=" + colspan + " style='color:" + event.colour + ";'>&nbsp;" + std['t'] + "</TD></TR>";
         }
...
function CalendarEvent(id,title,startTS,endTS,URL,location,repeatTS,repeatType,repeatInt,repeatByDay, colour){
   this.id = id;
   this.title = title;
   this.URL = URL;
