...
const MAX_UPLOAD = convert('50 GB');
...
function addCounters() {
  $x("//table[.//th]//tr[not(.//th)]").forEach(function(row, i) {
      ...
      var totalupstr = $xs("./td[last() - 1]/text()", row).data;
      var totalup = convert(totalupstr);
      console.log(totalupstr);
      
      var newcell = document.createElement('td');
      newcell.appendChild((new ProgressBar({comp: (total/MAX > 1) ? 1 : totalup / MAX_UPLOAD, foreground: ['#0f0', '#ff0', '#f00'], background: '#F9F3E7'}, 100, 20)).canvasElement); 
      if (i == 0) {
        setFavicon(total);
      }
