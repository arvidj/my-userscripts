...
(function() {
	...
	function scan_all_torrent_pages(type, page_cb, finish_cb) {
		...
		function request_url() {
			return '/torrents.php?type='+type+'&action=basic&userid='+whatcd_id+'&page='+page;
		}
		...
		function page_handler(response) {
			if (response.status == 200) {
				...
				/* Force basic search as advanced may be unparseable */
				page += 1;
	...
	/* Scan page if current page is snatched or uploaded */
	if (/what\.cd\/torrents\.php/.test(document.URL)) {
		/* Parse search */
		var search = {};
		var search_list = document.location.search.substring(1).split('&');
		...
		if ((search.type == 'snatched' || search.type == 'uploaded'
				|| search.type == 'seeding' || search.tyoe == 'leeching') && search.userid == whatcd_id) {
